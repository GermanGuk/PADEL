...
export default async function GamesAdminPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <h1 className="text-xl font-bold text-ink">Игры и турниры</h1>
        ...
        {games.map((game) => (
          ...
          >
            ...
            <div className="relative h-28 w-full shrink-0 overflow-hidden rounded-lg bg-surface sm:w-40">
              {game.image && <Image src={game.image} alt="" fill className="object-cover" unoptimized={game.image.startsWith("http")} />}
            </div>
            ...
            <div className="flex flex-1 flex-col gap-3">
              ...
              <label className="flex w-fit items-center gap-2 text-xs text-grey-1">
                ...
                Главная (featured) карточка — попадает на первый экран сайта
              </label>
      ...
      {games.map((game) => (
        <form key={`del-${game.id}`} action={deleteGameAction} className="-mt-4 flex justify-end">
          <input type="hidden" name="id" value={game.id} />
      ...
      <details className="rounded-2xl border border-dashed border-line p-5">
        <summary className="cursor-pointer text-sm font-medium text-ink">+ Добавить игру или турнир</summary>
        <form action={createGameAction} className="mt-4 flex flex-col gap-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="flex flex-col gap-1 text-xs text-grey-1">
              Бейдж (дата)
          ...
          <label className="text-xs text-grey-1">
            Фото: <input type="file" name="file" accept="image/*" />
          </label>
          <label className="flex w-fit items-center gap-2 text-xs text-grey-1">
